...
Cursor agent skill that produces a `SoundDefinition` for [@litlab/audx](../../packages/audio) from any input — a natural-language prompt, an audio file the user shares, or both.
...
- [`.themes/core.ts`](../../.themes/core.ts) — every `event-*` recipe template is taken from this first-party patch.
