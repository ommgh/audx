...
describe("theme-loader", () => {
	...
	describe("loadTheme", () => {
		it("should load the minimal theme successfully", async () => {
			const patch = await loadTheme("minimal");

			expect(patch).toBeDefined();
			expect(patch.name).toBe("Minimal");
			expect(patch.sounds).toBeDefined();
			expect(typeof patch.sounds).toBe("object");
		});

		it("should load the playful theme successfully", async () => {
			const patch = await loadTheme("playful");

			expect(patch).toBeDefined();
			expect(patch.name).toBe("Playful");
			expect(patch.sounds).toBeDefined();
			expect(typeof patch.sounds).toBe("object");
		});
		...
		it("should include expected sound definitions in minimal theme", async () => {
			...
			expect(patch.sounds.click).toBeDefined();
			expect(patch.sounds.success).toBeDefined();
			expect(patch.sounds.error).toBeDefined();
			expect(patch.sounds.hover).toBeDefined();
		});
		...
		it("should include metadata in the patch", async () => {
			const patch = await loadTheme("minimal");

			expect(patch.name).toBeDefined();
			expect(typeof patch.name).toBe("string");
			expect(patch.author).toBeDefined();
		...
		it("should validate patch structure", async () => {
			// This test ensures the loaded patch has the expected structure
			const patch = await loadTheme("minimal");

			// Validate sounds object
			expect(patch.sounds).toBeDefined();
			expect(typeof patch.sounds).toBe("object");
			expect(Object.keys(patch.sounds).length).toBeGreaterThan(0);

			// Validate at least one sound definition
			const firstSound = Object.values(patch.sounds)[0];
