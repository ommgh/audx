...
import __node_cjsPath, { basename, isAbsolute, join, resolve } from "node:path";
...
async function ensureConfig() {
	...
	require$1("node:fs").writeFileSync(
		configPath(),
...
function getPatchesDir() {
	...
	const output =
		(_ref = config == null ? void 0 : config.output) != null ? _ref : ".themes";
...
async function getInstalledPatches() {
	...
	for (const file of files) {
		...
		try {
			...
			const exportCount = (
				(_raw_match = raw.match(/^export const /gm)) != null ? _raw_match : []
			).length;
			const name =
				(_ref = nameMatch == null ? void 0 : nameMatch[1]) != null
					? _ref
					: basename(file, ".ts");
			patches.push({
				file,
...
function generateModule(data) {
	...
	const { sounds: _, $schema: _s } = data,
		meta = _object_without_properties_loose(data, ["sounds", "$schema"]);
...
async function addFromRegistry(options) {
	...
	if (options.patch) {
		...
		const match = index.find(
			(e) => e.name.toLowerCase() === patchName.toLowerCase(),
		);
		if (!match) {
...
async function find(args) {
	...
	const matches = query
		? index.filter((entry) => {
				var _entry_tags;
				const haystack =
					`${entry.name} ${entry.description} ${((_entry_tags = entry.tags) != null ? _entry_tags : []).join(" ")}`.toLowerCase();
...
async function remove(args) {
	...
	for (const file of files) {
		try {
			...
			removed.push(
				(_ref = pk == null ? void 0 : pk.name) != null ? _ref : file,
			);
		} catch (err) {
